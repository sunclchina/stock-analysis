 ...
 * - 固定240px / 可折叠至60px，过渡动画0.3s
 * - 顶部：K线抽象Logo + 系统名称
 * - 中部：7个核心导航项（仪表盘/实时行情/智能选股/智能分析/智能预警/操盘笔记/系统配置）
 * - 底部：折叠按钮 / 主题切换 / 帮助中心 / 版权标注
 * - 悬停渐变、当前标亮、预警角标
 */
...
const navItems: NavItemDef[] = [
  ...
  { key: 'warning', label: '智能预警', path: '/warning', icon: <AlertOutlined />, badge: 0 },
...
const Sidebar: React.FC<SidebarProps> = ({ themeMode, onToggleTheme, onOpenHelp }) => {
  ...
  const menuItems = navItems.map((item) => {
    ...
    return {
      ...
      icon: item.badge !== undefined && item.badge > 0 ? (
        <Badge count={item.badge} size="small" color="#FF4D4F" offset={[4, -4]}>
          {item.icon}
        </Badge>
      ) : (
        item.icon
  ...
  if (isAdmin()) {
    menuItems.push({
      key: adminNav.key,
      icon: adminNav.icon,
      label: <span>{adminNav.label}</span>,
    });
  }
  ...
  return (
    <Sider
      ...
      className="sidebar"
    >
      {/* 内层 flex 容器（Ant Design Sider 不支持直接 flex，用内层控制） */}
      ...
      >
        {/* Logo图片 - K线抽象图标 */}
        <img src="/logo.svg" alt="logo" style={{ width: 60, height: 60, flexShrink: 0, objectFit: 'contain', cursor: 'pointer' }} onClick={(e) => { e.stopPropagation(); navigate('/logo'); }} />
