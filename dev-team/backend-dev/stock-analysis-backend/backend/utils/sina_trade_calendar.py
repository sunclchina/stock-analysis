...
从新浪财经获取全年A股交易日数据，使用 py_mini_racer 解码。
不经过 AKShare，直接调用原始 API：
  https://finance.sina.com.cn/realstock/company/klc_td_sh.txt

缓存策略：进程内缓存，每次启动仅获取一次。
"""
...
# 进程内缓存
_trade_date_cache: Optional[list[date]] = None
_cache_timestamp: Optional[datetime] = None


def _fetch_and_decode() -> list[date]:
    """
    直连新浪交易日历 API，获取并解码全年交易日列表。

    Returns:
        list[date]: 交易日列表（升序排列）
    """
    url = "https://finance.sina.com.cn/realstock/company/klc_td_sh.txt"
    resp = requests.get(url, timeout=10)
    resp.encoding = "utf-8"

    # 提取编码后的字符串：var datelist="..." 中的引号内容
    raw = resp.text
    encoded = raw.split("=")[1].split(";")[0].replace('"', "").strip()

    # 内联JS解码函数（来自新浪原始API的Reverse Engineer）
    js_decode_func = _get_js_decode()
    js = py_mini_racer.MiniRacer()
    js.eval(js_decode_func)
    decoded_list = js.call("d", encoded)

    # 转为 date 对象列表（JS解码返回 ISO 字符串或 datetime）
    ...
    for item in decoded_list:
        ...
        if isinstance(item, datetime):
            ...
            # ISO 格式: "2026-05-18T00:00:00.000Z"
            try:
                dt = datetime.fromisoformat(item.rstrip("Z")).date()
            except (ValueError, TypeError):
                logger.warning(f"交易日历解析：无法解析日期字符串 {item}")
        if dt:
            trade_dates.append(dt)

    # 补充已知缺失的交易日（AKShare 也做了同样的事）
    missing_dates = [
        date(1992, 5, 4),
    ]
    trade_dates.extend(d for d in missing_dates if d not in trade_dates)

    trade_dates = sorted(set(trade_dates))
    logger.info(f"新浪交易日历：共 {len(trade_dates)} 个交易日")
    return trade_dates


def _get_js_decode() -> str:
    """新浪交易日历数据解码 JS 函数（内联，不依赖外部包）。"""
    return """function d(t) {
...
def _get_trade_dates() -> list[date]:
    """获取交易日列表（带进程内缓存）"""
    global _trade_date_cache, _cache_timestamp
    if _trade_date_cache is None:
        try:
            _trade_date_cache = _fetch_and_decode()
            _cache_timestamp = datetime.now()
            logger.info(f"交易日历缓存已更新（{len(_trade_date_cache)} 条）")
        except Exception as e:
            logger.warning(f"新浪交易日历获取失败: {e}，回退到周末判断")
            _trade_date_cache = None
    return _trade_date_cache


def is_trading_day(today: Optional[date] = None) -> bool:
    """
    判断指定日期是否为A股交易日。

    使用新浪交易日历 API 直接查询，缓存全年数据。
    若 API 不可用，回退到简单的周末判断。
